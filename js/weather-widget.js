...
weatherWidget.getWeatherIcon = function(code, icon) {
    var iconStyle = "";
...
weatherWidget.getWeatherInfo = function(weatherCityID) {
    ...
    $.getJSON("http://api.openweathermap.org/data/2.5/weather?id=" + weatherCityID + "&APPID=" + apiKeys.openWeatherMapKey, function (data) {
        ...
        var speedString = "";
        if (speed == "MPH") {
            speedString = bunnyHelpers.msToMph(data.wind.speed) + " " + speed;
        }
        else if (speed == "m/s") {
        ...
        var tempString = "";
        if (deg == "C") {
            tempString = bunnyHelpers.kelvinToCelsius(data.main.temp) + "&deg;" + deg;
        }
        else if (deg == "F") {
            tempString = bunnyHelpers.kelvinToFahrenheit(data.main.temp) + "&deg;" + deg;
        }
        ...
        html += '</div>';
        html += '<div class="row">';
        html += '<div class="col-md-6"><i class="weather-icon wi ' + weatherWidget.getWeatherIcon(data.weather[0].id, data.weather[0].icon) + '"></i></div>';
    ...
    $.getJSON("http://api.openweathermap.org/data/2.5/forecast?id=" + weatherCityID + "&APPID=" + apiKeys.openWeatherMapKey, function (data) {
        var html = "";
        ...
        for (i = 0; i < 3; i++) {
            var tempString = "";
            if (deg == "C") {
                tempString = bunnyHelpers.kelvinToCelsius(data.list[i].main.temp) + "&deg;" + deg;
            }
            else if (deg == "F") {
                tempString = bunnyHelpers.kelvinToFahrenheit(data.list[i].main.temp) + "&deg;" + deg;
            }

            var iconString = weatherWidget.getWeatherIcon(data.list[i].weather[0].id, data.list[i].weather[0].icon);
            ...
            html += '<div class="row no-margin">';
                html += '<div class="col-md-3"></div>';
                    ...
                    html += '<div class="row">';
                    html += '<div class="col-md-6"><h4>' + weatherWidget.getForecastTimeString (fcTime.getHours(), twentyFourHour) + '</h4></div>';
                    // html += '</div>';
                    // html += '<div class="row">';
...
weatherWidget.getForecastTimeString = function(hoursIn, twentyFourHour) {
    var hoursText = "";
